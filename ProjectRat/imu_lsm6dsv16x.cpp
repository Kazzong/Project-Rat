...
bool LSM6DSV16X::configureMl() {
  // A trained MLC/FSM configuration must be generated by Unico and loaded
  // here before the result register can be treated as meaningful.
  _mlEnabled = false;
  return false;
}
...
bool LSM6DSV16X::readMlState(uint8_t& mlResult) {
  ...
  if (!readRegister(IMU_MLC1_SRC, &mlResult, 1)) {
    return false;
  }
...
bool LSM6DSV16X::readMlChangeStatus(uint8_t& status) {
  if (!_mlEnabled) {
    return false;
  }

  return readRegister(IMU_MLC_STATUS, &status, 1);
}
